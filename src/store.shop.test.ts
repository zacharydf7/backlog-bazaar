...
beforeEach(() => {vi.clearAllMocks(); useStore.setState({userId:'buyer',cloud:true,coins:500,shopPurchasedIds:[],shopItems:[],error:null});});
describe('cosmetic checkout', () => {
  it('uses the confirmed price and server balance', async () => {
    ...
    expect(useStore.getState().shopPurchasedIds).toEqual(['item']);
  });

...
const CollectionDraftManager = lazy(() => import("./CollectionDraftManager").then((module) => ({ default: module.CollectionDraftManager })));
...
export function ShopManager() {
  ...
  const canCollections = useStore((s) => s.can("shop.collections.drafts") || s.can("shop.collections.publish"));
  ...
  if (collections) return <Suspense fallback={<p role="status" className="text-sm text-muted">Loading collection drafts…</p>}><CollectionDraftManager onClose={() => setCollections(false)} /></Suspense>;
  return (
